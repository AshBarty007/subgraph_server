...
async function test(){
   ...
   await DB.updateData("TestTable",{name:"first"},{name:"second"}).catch((err)=>{console.log("update err",err)})
   await DB.findData("TestTable",{name:"second"}).then((data)=>{console.log("2.data",data)})
   await DB.deleteData("TestTable",{name:"second"},true).catch((err)=>{console.log("delete err",err)})
